...
class MelCodeRunnerApp:
    def __init__(self, root):
        ...
        self.keyword_patterns = [
            r'\b(if|else|while|for|switch|case|default|return|break|continue|do)\b',
        ]
        self.datatype_patterns = [
            r'\b(int|float|char|string|bool|void|const)\b',
        ]
    ...
    def create_main_layout(self):
        ...
        notebook.add(quads_frame, text="Quadruples")

        quads_header_frame = tk.Frame(quads_frame, bg=self.bg_color)
        quads_header_frame.pack(fill=tk.X)
        
        quads_header = tk.Label(quads_header_frame, text="Quadruples", bg=self.bg_color, fg=self.text_color, font=("Arial", 12, "bold"))
        quads_header.pack(fill=tk.X, padx=5, pady=5)
    ...
    def _adjust_font(self, step):
        ...
        if hasattr(self, "line_numbers"):
            self.line_numbers.configure(font=self.code_font)
    ...
    def highlight_syntax(self):
        ...
        # find & tag strings / char literals
        string_ranges = []
        str_pat = r'"[^"\\]*(?:\\.[^"\\]*)*"|\'[^\'\\]*(?:\\.[^\'\\]*)*\''
        for m in re.finditer(str_pat, content):
            start_off, end_off = m.start(), m.end()
            string_ranges.append((start_off, end_off))
        ...
        for m in re.finditer(r'//.*$', content, re.MULTILINE):
            start_off, end_off = m.start(), m.end()
            comment_ranges.append((start_off, end_off))
            start = f"1.0+{start_off}c"
            end   = f"1.0+{end_off}c"
            text.tag_add("comment", start, end)
        text.tag_configure("comment", foreground="#6A9955")

        # helper to test if an offset is inside any protected range (comment or string)
        def in_protected(off):
            for a,b in comment_ranges + string_ranges:
                if a <= off < b:
                    return True
            return False

        # tag keywords (skip if in a comment/string)
        for i, pat in enumerate(self.keyword_patterns):
            tag = f"kw{i}"
            text.tag_configure(tag,
                            foreground="#a53fd1",
                            font=(self.code_font.cget("family"),
                                    self.code_font.cget("size"),
                                    "bold"))
        ...
        # tag datatypes (skip if in a comment/string)
        for i, pat in enumerate(self.datatype_patterns):
            tag = f"dt{i}"
            text.tag_configure(tag,
                            foreground="#3251c9",
                            font=(self.code_font.cget("family"),
    ...
    def execute_code(self, temp_file):
        try:
            ...
            compiler = os.path.join(base_path, "mellow.exe")

            cmd = [compiler, temp_file]
    ...
    def getReady(self):
        ...
        def on_key_press(event):
            if event.char == "{":
                pos = app.code_text.index(tk.INSERT)
                app.code_text.insert(pos, "}")
                app.code_text.mark_set(tk.INSERT, f"{pos}")
            if event.char == "(":
                pos = app.code_text.index(tk.INSERT)
                app.code_text.insert(pos, ")")
                app.code_text.mark_set(tk.INSERT, f"{pos}")
        ...
        app.code_text.bind("<KeyPress>", on_key_press)
        app.code_text.bind("<KeyRelease>", on_key_release)
